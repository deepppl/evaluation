...
def test(posterior, config, backend="pyro", mode="mixed"):
    ...
    try:
        if backend == "pyro":
            pyro_model = PyroModel(stanfile, mode=mode, compiler= [stanc], recompile=True)
        else:
            pyro_model = NumPyroModel(stanfile, mode=mode, compiler= [stanc], recompile=True)
...
def test_all(backend, mode, config):
    ...
    with open(logpath, "a") as logfile:
        ...
        for name in my_pdb.posterior_names():
            ...
            log(logfile, res)
    return { 'success': success, 'compile_error': compile_error, 'inference_error': inference_error }
