...
def make_plots(left, right, path, ticks = None):
  plt.hist(left, bins=10, histtype='stepfilled', alpha=0.75, label=left.name, color=colors[left.name])
  plt.hist(right, bins=10, histtype='stepfilled', alpha=0.5, label= right.name, color=colors[right.name])
...
def model_hand_coded(batch_size, imgs, labels, mlp):
    prior = {
      'l1.weight' : dist.Normal(torch.zeros(mlp.l1.weight.shape), torch.ones(mlp.l1.weight.shape)),
      'l1.bias' : dist.Normal(torch.zeros(mlp.l1.bias.shape), torch.ones(mlp.l1.bias.shape)),
      'l2.weight' : dist.Normal(torch.zeros(mlp.l2.weight.shape), torch.ones(mlp.l2.weight.shape)),
      'l2.bias' : dist.Normal(torch.zeros(mlp.l2.bias.shape), torch.ones(mlp.l2.bias.shape))
    }
...
if __name__ == "__main__":
  ...
  lr = 0.01
  posterior_size = 100
  svi = pyro.infer.SVI(model=model_hand_coded,
  ...
  posterior_hand_made = generate_posterior(svi, posterior_size, (None, None, None, mlp_hand_coded))
  accuracies_hand_made = []
  for j, data in enumerate(test_loader, 0):
      images, labels = data
      accuracy = (predict(images, posterior_hand_made) == labels).type(torch.float).mean()
  ...
  for epoch in tqdm.tqdm(range(epochs)):  # loop over the dataset multiple times
      for j, (imgs, lbls) in enumerate(train_loader, 0):
          # calculate the loss and take a gradient step
          loss = svi.step(batch_size=batch_size, nx=nx, nh=nh, ny=ny, imgs=imgs, labels=lbls+1, mlp=mlp)
          losses.append(loss)
  posterior = svi.posterior(posterior_size, batch_size=batch_size, nx=nx, nh=nh, ny=ny, imgs=imgs, labels=lbls+1, mlp=mlp)
  ...
  for j, data in enumerate(test_loader, 0):
      images, labels = data
      agreement = (predict(images, posterior_hand_made) == predict(images, posterior)).type(torch.float).mean()
      agreements.append(agreement)

  agreements = pd.Series([x.item() for x in agreements])
  print('Agreement')
  print(agreements.describe())
  df = pd.DataFrame({DeepStanSVIName : accuracies, PyroName : accuracies_hand_made, 'agreement' : agreements})
