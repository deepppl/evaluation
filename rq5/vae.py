...
def evaluate(encoder, name, pair_star, test_loader):
  ...
  print(f"{name}  precision: {precision}, recall: {recall}, f1: {f1(precision, recall)}")

def train_and_evaluate(epochs, name, pair_star, svi, encoder, decoder, train_loader, test_loader):
  for epoch in tqdm.tqdm(range(epochs)):  # loop over the dataset multiple times
    for j, (imgs, _) in enumerate(train_loader, 0):
      # calculate the loss and take a gradient step
      k = len(imgs)
      loss = svi.step(batch_size=k, nz=nz, x=imgs, encoder=encoder, decoder=decoder)
  evaluate(encoder, name, pair_star, test_loader)

def build_and_evaluate(epochs, name, builder, params, pair_star, train_loader, test_loader):
  encoder, decoder = build_vae()
  svi = builder(params)
  train_and_evaluate(epochs, name, pair_star, svi, encoder, decoder, train_loader, test_loader)
...
def model_pyro(*, batch_size, nz, x, encoder, decoder):
  ...
  latent = pyro.sample('latent', dist.Normal(torch.zeros(nz), torch.ones(nz), batch_size))
  loc_img = decoder(latent)
  pyro.sample('x', dist.Bernoulli(loc_img), obs=x)

def build_deep_stan_svi(params):
  model = dppl.PyroModel(
        model_code)
...
if __name__ == "__main__":
  ...
  params = {'lr' : 0.01}
  build_and_evaluate(args.epochs, 'DeepStanSVI', build_deep_stan_svi,
                     params, pair_star, train_loader, test_loader)
  pyro.clear_param_store()
  build_and_evaluate(args.epochs, 'Pyro', build_pyro_svi,
                    params, pair_star, train_loader, test_loader)
